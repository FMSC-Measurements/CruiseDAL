        ...
        protected override void DoUpdate(DbConnection conn, DbTransaction transaction, IExceptionProcessor exceptionProcessor)
        {
            ...
            RebuildTable(conn, transaction, exceptionProcessor, new TreeDefaultValueTableDefinition_3_5_5(),
                new[] {
                    ...
                    new KeyValuePair<string, string>("TreeGrade", "CASE TreeGrade WHEN NULL THEN NULL ELSE nullif(substr(trim(TreeGrade),-1,1), '') END"),
                    new KeyValuePair<string, string>("TreeGradeDead", "CASE TreeGradeDead WHEN NULL THEN NULL ELSE nullif(substr(trim(TreeGradeDead),-1,1), '') END"),
